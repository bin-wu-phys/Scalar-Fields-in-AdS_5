...
int main(){
	...
	for(int i=0;i<dd.col();i++) dd[n-1][i]*=-3.0;
	dd=dd-((2.0*(3.0+cos(2*xxx))/sin(2.0*xxx))&0);
	d.LUdcmp();

	// Initial conditions
	DoubleVector II=(0&(2.0*EPS*exp(-4.0/(PI*PI*SIG*SIG*(sin(2.0*x.sub(1,n-1))^2)))/PI)&0),Phi=II;//((2.0*EPS*exp(-4.0*(cot(x.sub(0,n-1))^2)/(PI*PI*SIG*SIG))/PI)&0);//
	...
	ddelta=(sin(2.0*xx)*( ((Phi*Phi)+(II*II)).sub(1,n) )/3.0 );
	delta=(0&(d|ddelta));
	Ap=(dd-ddelta);
	Ap.LUdcmp();
	a=(0&(Ap|(ddelta)));
	...
	dAnm2=( (2.0/3.0)*sin(2.0*x)*A*A*exp(-delta)*Phi*II );
	...
	dAnm1=( (2.0/3.0)*sin(2.0*x)*A*A*exp(-delta)*Phi*II );
